...
class TestAccountDynamicRules(TransactionCase):
    ...
    def setUp(self):
        ...
        # Must include invoice_repartition_line_ids with a distinct account so Odoo 16
        # does not inherit the base line's account into the tax line (which would
        # cause "Dynamic rules must not override VAT/tax line accounts" errors).
